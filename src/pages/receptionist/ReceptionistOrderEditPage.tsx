...
const ReceptionistOrderEditPage: React.FunctionComponent = () => {
    ...
    if (tests.length + profiles.length > 0) {
      ...
      firestore
        ...
        .then(async () => {
          ...
          history.push(
            `/receptionist-panel/patients/${idPatient}?order=${idOrder}`
          );
        })
        .catch((error) => {
          setIsLoading(false);
        ...
        <>
          <Breadcrumbs
            navigations={[
              ...
              {
                uri: `/receptionist-panel/patients/${idPatient}`,
...
export default ReceptionistOrderEditPage;
